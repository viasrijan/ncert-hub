...
export async function generateMetadata({ params }: { params: Promise<{ classNum: string }> }): Promise<Metadata> {
  ...
  return { title: `Solutions - Class ${toRoman(Number(classNum))}`, description: `NCERT Solutions for Class ${toRoman(Number(classNum))} — external, legal-safe.`, robots: { index: false, follow: false } }
}
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col items-center gap-6 px-4 py-8 md:px-8 md:py-12">
      ...
      <div className="flex flex-col items-center text-center gap-2 animate-fade-in-up">
        <h1 className="font-display text-5xl font-bold tracking-tight md:text-6xl text-foreground">Class {toRoman(c)}</h1>
