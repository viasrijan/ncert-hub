...
export function RecentsStrip() {
  ...
  return (
    <section aria-labelledby="recents-heading" className="flex flex-col items-center gap-4 animate-fade-in-up animate-delay-100 w-full">
        ...
        {recents.slice(0, 8).map((r) => {
          ...
          return (
            <Link
              key={r.pdfCode}
              href={`/read/${r.pdfCode}`}
              className="flex w-64 shrink-0 items-center gap-3.5 rounded-2xl border border-border/30 bg-card/80 backdrop-blur-sm p-3.5 shadow-card transition-all duration-200 hover:shadow-elevated hover:border-gold/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            >
